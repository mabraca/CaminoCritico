...
public class EdgeWeightedDigraph {
    ...
    public EdgeWeightedDigraph(EdgeWeightedDigraph G) {
        ...
        for (int v = 0; v < G.V(); v++) {
            ...
            LinkedList<DirectedEdge> reverse = new LinkedList<DirectedEdge>();
     ...
     * @return the number of edges in this edge-weighted digraph
     */
    public boolean tienePredecesor(int nodo) {
        boolean tiene=false;
        for(DirectedEdge i: edges()){
            if(i.to()== nodo) tiene=true;
        }
        return tiene;
    }
    public boolean tieneSucesor(int nodo) {
        boolean tiene=false;
        for(DirectedEdge i: edges()){
            if(i.from()== nodo) tiene=true;
        }
        return tiene;
    }
    ...
    public LinkedList<Integer> ady(int v) {
        LinkedList<Integer> adyacentes = new LinkedList<Integer>();
        validateVertex(v);
        for(DirectedEdge i: adj[v])
            adyacentes.add(i.to());
        return adyacentes;
    }
    ...
    public LinkedList<DirectedEdge> sucesores(int w) {
        LinkedList<DirectedEdge> list = new LinkedList<DirectedEdge>();
        for (int v = 0; v < V; v++) {
            for (DirectedEdge e : adj(v)) {
                if(e.to()==w)list.add(e);
            }
        }
        return list;
    } 
